...
def preprocess_data(df, scaler, tfidf, features_for_splitting):
    ...
    if numerical_features_to_scale:
         try:
              ...
              # Access dtype correctly for each column
              cols_to_scale_df = df[numerical_features_to_scale]
              if not cols_to_scale_df.empty:
                  df[numerical_features_to_scale] = scaler.transform(cols_to_scale_df.values) # Use .values to get NumPy array
              else:
                   st.info("No numerical features to scale in the current data.")
