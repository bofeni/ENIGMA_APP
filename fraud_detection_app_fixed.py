...
def preprocess_data(df, scaler, tfidf, features_for_splitting):
    try:
        ...
        # Fixed: Properly check for object dtype columns
        categorical_cols_to_encode = []
        for col in df.columns:
            if col in df.columns and col not in date_cols + ['Customer_Name', 'Customer_Email', 'Customer_Phone', 'Adjuster_Notes', 'Policy_Number', 'Claim_ID']:
                # Check if the column exists and has object dtype
        ...
        # Only select columns that exist in features_for_splitting
        available_features = [col for col in features_for_splitting if col in df.columns]
        df = df[available_features]
        
        # If some features are missing, add them with default values
        missing_features = set(features_for_splitting) - set(available_features)
        ...
        # More conservative approach to identify numerical features to scale
        numerical_features_to_scale = []
        for col in features_for_splitting:
            if col in numerical_features_original and col in df.columns:
                if pd.api.types.is_numeric_dtype(df[col]):
                    numerical_features_to_scale.append(col)

        if numerical_features_to_scale:
             try:
                  # Ensure all columns exist and contain valid numerical data
                  df[numerical_features_to_scale] = df[numerical_features_to_scale].fillna(0)
        ...
        # Return a dataframe with the expected features filled with zeros as fallback
        fallback_df = pd.DataFrame(0, index=range(len(df)), columns=features_for_splitting)
        return fallback_df
...
- Customer_Name, Location (for network analysis)
- Adjuster_Notes (for sentiment and text analysis) 
- Incident_Date, Claim_Submission_Date (for timing analysis)
- Policy_Start_Date, Policy_End_Date (for policy duration)
- Policy_Number (for claim frequency)
- Claim_Amount, Premium_Amount (for amount analysis)
- Customer_Age (for demographics)

*Note: Missing columns will be filled with default values, but predictions may be less accurate.*
""")
...
if uploaded_file is not None:
    try:
        ...
        result_df['Predicted_Fraud_Flag'] = predictions
        result_df['Predicted_Fraud_Probability'] = predictions_proba
        ...
        display_columns = []
        for col in ['Claim_ID', 'Policy_Number', 'Customer_Name', 'Claim_Amount']:
            if col in result_df.columns:
                display_columns.append(col)
        display_columns.extend(['Predicted_Fraud_Flag', 'Predicted_Fraud_Probability'])
        
        st.dataframe(result_df[display_columns])

        # Summary of predictions
        fraud_count = result_df['Predicted_Fraud_Flag'].sum()
        total_claims = result_df.shape[0]
